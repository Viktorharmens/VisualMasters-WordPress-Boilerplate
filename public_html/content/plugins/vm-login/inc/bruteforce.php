<?php
	...
	require_once 'vendor/htaccess.php';
	...
	class BruteForceLoginProtection {
	    ...
	    public function showSettingsPage() {
	        ...
	        $this->__fillOptions();
	        include 'vendor/settings-page.php';
	    }
	    ...
	    public function loginFailed() {
	        ...
	        if (!in_array($IP, $whitelist)) {
	            ...
	            if ($denyIP) {
	                ...
	                wp_redirect( $referer[0] . '?login=failed&blocked=true' );
	                exit();
	            }
				...
				if ( !empty($referer[0]) && !strstr($referer[0],'wp-login') && !strstr($referer[0],'wp-admin') ) {
					wp_redirect( $referer[0] . '?login=failed&attempts=' . $remainingAttempts );
					exit;
				}
	    ...
	    private function __setDefaultOptions() {
	        $this->__options = array(
	            ...
	            '403_message'        => 'This IP has been blacklisted for at least 60 minutes due to reaching the maximum amount of login attempts.', //Message to show to a blocked user
